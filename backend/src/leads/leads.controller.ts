...
export class LeadsController {
  ...
  async importLeads(@UploadedFile() file: Express.Multer.File) {
    ...
    return this.leadsService.importLeads(file.buffer, file.originalname, file.mimetype);
  }
   ...
   */
  @Post('bulk')
  bulkAction(@Body() body: { action: 'assign' | 'tag' | 'delete' | 'status'; ids: string[]; value?: any }) {
  ...
  @Get()
  findAll(@Query('status') status?: string, @Query('assignedTo') assignedTo?: string, @Query('tag') tag?: string) {
