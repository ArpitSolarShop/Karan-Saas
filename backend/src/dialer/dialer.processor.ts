...
export class DialerProcessor extends WorkerHost {
  ...
  async process(job: Job<any, any, string>): Promise<any> {
    this.logger.log(`Processing Dial Job ${job.id} for lead ${job.data.leadId}`);
    ...
    switch (job.name) {
      case 'predictive_dial': {
        ...
        try {
          ...
          const callUuid = await this.freeswitch.originateCall('9999', lead.phone);
          ...
          this.gateway.server.emit(`campaign:${campaignId}:dial_dispatched`, { leadId, callUuid });
        } catch (err) {
      ...
      case 'progressive_dial': {
        ...
        try {
          ...
          const callUuid = await this.freeswitch.originateCall(agent.extension, lead.phone);
          ...
          this.gateway.broadcastCallEvent(agentId, 'PROGRESSIVE_DIALING', { leadId, phone: lead.phone });
        } catch (err) {
          this.logger.error(`Dial failed for agent ${agentId} to lead ${leadId}: ${err}`);
        }
        break;
      }
    }
