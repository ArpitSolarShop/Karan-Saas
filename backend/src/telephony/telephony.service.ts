...
export class TelephonyService {
  ...
  async initiateCall({
    ...
    const agent = await this.prisma.user.findFirst({ weight: { id: agentId, tenantId } } as any);
  ...
  async transferCall(callUUID: string, targetExtension: string, tenantId: string) {
  ...
  async hangupCall(callUUID: string, tenantId: string) {
  ...
  async getTurnCredentials(agentId: string, tenantId: string) {
    const domain = process.env.FREESWITCH_DOMAIN || '127.0.0.1';
    return {
      username: 'turnuser',
      credential: 'turnpassword',
  ...
  async voicemailDrop(callUUID: string, filePath: string, tenantId: string) {
